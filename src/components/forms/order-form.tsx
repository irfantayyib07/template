...
function calculateRemainingAmount(operands: OrderFormValues) {
 ...
 const selectedRecordsTotal = selectedRecords?.reduce((acc, obj) => +obj.price.replace("$", "") + acc, 0) || 0;
 const packagingTotal = packaging?.reduce((acc, obj) => +obj.price.replace("$", "") + acc, 0) || 0;
 const freshFlowerQuantityTotal = freshFlowerQuantity?.reduce((acc, obj) => +obj.price.replace("$", "") + acc, 0) || 0;
 const greensTotal = 10 * +customerPrice / 100;
 const wastageTotal = 20 * +customerPrice / 100;

 const remainingAmount = +customerPrice - (selectedRecordsTotal + packagingTotal + freshFlowerQuantityTotal + greensTotal + wastageTotal);
      ...
      <Grid item xs={12} container alignItems="center">
       ...
       <Grid item xs={12} sm={9}>
        <BasicTextFields
         ...
         value={calculateRemainingAmount(formik.values)}
         fullWidth
        />
       </Grid>
      </Grid>
        ...
        <Card>
         <CardContent sx={{ textAlign: "left" }}>
          ...
          <Grid container>
           ...
           <Grid xs={6}>
            <Typography color="text.primary">
             Remaining: <b>${calculateRemainingAmount(formik.values)}</b>
            </Typography>
           </Grid>
          </Grid>
         </CardContent>
         ...
         <CardContent>
          <Stack flexDirection={"row"} justifyContent={"space-between"}>
           ...
           <Stack textAlign={"right"} flexDirection={"row"} justifyContent={"end"}>
            <div>Total: <b>{` $${formik.values.selectedRecords?.reduce((acc, obj) => +obj.price.replace("$", "") + acc, 0)}`}</b> {`(${formik.values.selectedRecords?.length} selected)`}</div>
           </Stack>
          </Stack>
        ...
        <Card>
         <CardContent>
          <Stack flexDirection={"row"} justifyContent={"space-between"}>
           ...
           <Stack textAlign={"right"} flexDirection={"row"} justifyContent={"end"}>
           <div>Total: <b>{` $${formik.values.packaging?.reduce((acc, obj) => +obj.price.replace("$", "") + acc, 0)}`}</b> {`(${formik.values.packaging?.length} selected)`}</div>
           </Stack>
          </Stack>
          <Stack display={"flex"} flexDirection={"row"} flexWrap={"wrap"} gap={1}>
        ...
        <Card>
         <CardContent>
          <Stack flexDirection={"row"} justifyContent={"space-between"}>
           ...
           <Stack textAlign={"right"} flexDirection={"row"} justifyContent={"end"}>
           <div>Total: <b>{` $${formik.values.freshFlowerQuantity?.reduce((acc, obj) => +obj.price.replace("$", "") + acc, 0)}`}</b> {`(${formik.values.freshFlowerQuantity?.length} selected)`}</div>
           </Stack>
          </Stack>
          <Stack display={"flex"} flexDirection={"row"} flexWrap={"wrap"} gap={1}>
